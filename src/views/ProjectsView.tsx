      ...
      <div className="flex flex-wrap items-center justify-between gap-3 sm:gap-4 pb-4 border-b border-slate-200 dark:border-slate-800/80">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <h1 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white tracking-tight">Projects & Sprint Roadmaps</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
        {appState.projects.map(project => {
          ...
          return (
            <div
              key={project.id}
              className="rounded-xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/90 p-4 sm:p-5 flex flex-col justify-between hover:border-indigo-300 dark:hover:border-slate-700 transition-all space-y-4 group shadow-xs hover:shadow-sm"
            >
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-mono capitalize ${
                    project.status === 'active' ? 'bg-emerald-50 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/30' :
                    project.status === 'at_risk' ? 'bg-rose-50 dark:bg-rose-500/15 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-500/30' :
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 text-[11px] space-y-1">
                  <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 font-mono">
                    ...
                    <span className={budgetPercent > 90 ? 'text-rose-600 dark:text-rose-400 font-bold' : 'text-slate-700 dark:text-slate-300'}>
                      {budgetPercent}%
                    </span>
                  </div>
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  {lead?.avatar && (
                    <img src={lead.avatar} alt={lead.name} className="w-5 h-5 rounded-full object-cover ring-1 ring-slate-200 dark:ring-slate-700" />
                  )}
                  <span className="text-slate-600 dark:text-slate-300 text-[11px] truncate max-w-[110px]">{lead?.name || 'Lead'}</span>
                </div>

                <button
                  onClick={() => onFilterTasksByProject(project.id)}
                  className="flex items-center gap-1 text-[11px] font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs">
          ...
          >
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
              <span className="font-bold text-slate-900 dark:text-white text-sm">Create New Project Initiative</span>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="sm:col-span-2">
                <label className="block text-slate-700 dark:text-slate-300 mb-1 font-semibold">Project Name *</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div>
                <label className="block text-slate-700 dark:text-slate-300 mb-1 font-semibold">Project Key (Short Prefix) *</label>
                <input
                  type="text"
                  required
                  value={key}
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white font-mono uppercase placeholder:text-slate-400 focus:outline-none focus:border-indigo-500"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-2.5 py-2 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-800 dark:text-slate-200 focus:outline-none focus:border-indigo-500"
                >
              ...
              <div>
                <label className="block text-slate-700 dark:text-slate-300 mb-1">Target Launch Date</label>
                <input
                  type="date"
                  value={targetDate}
              ...
              <div>
                <label className="block text-slate-700 dark:text-slate-300 mb-1">Budget Allocation ($)</label>
                <input
                  type="number"
                  step={1000}
